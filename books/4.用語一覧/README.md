...
ローカルの履歴がリモートの履歴とコンフリクトする場合、エラーになりプッシュは失敗してしまいます。詳しくは[コンフリクト](/4.用語一覧/#conflict（コンフリクト）)を参照してください。
...
![マージ1](../.vuepress/images/image13.png)

現在のブランチへ他のブランチの更新を取り込む処理のことです。

開始地点や丸はコミットです。

Aさんの操作しているブランチAにBさんの操作しているブランチBの進捗を取り込みたくなった時に使用するのがマージです。
BさんのブランチBを取り入れることで右の図のような状態になります。



## conflict（コンフリクト）

自分が変更を行った部分が他人の変更点と重複しまうことで、コンフリクト（衝突）が起きてしまいます。

![プッシュエラー](../.vuepress/images/image7.png)

コンフリクトが起きた場合、Githubのページに移動し、diff（差分）を確認しながら解決することが出来ます。
